        ...
        protected fun genBranchTargets(): PromisedValue {
            with(codegen) {
                ...
                for ((thenExpression, label) in expressionToLabels) {
                    mv.visitLabel(label)
                    thenExpression.accept(codegen, data).coerce(expression.asmType, expression.type).materialized
                    mv.goTo(endLabel)
                }
                mv.visitLabel(defaultLabel)
                val stackValue = elseExpression?.accept(codegen, data) ?: voidValue
                val result = stackValue.coerce(expression.asmType, expression.type).materialized
                mv.mark(endLabel)
                return result
            }
        }
