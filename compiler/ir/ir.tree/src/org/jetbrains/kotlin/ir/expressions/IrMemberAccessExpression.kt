...
fun IrMemberAccessExpression.putArgument(function: IrFunction, parameter: IrValueParameter, argument: IrExpression) = when {
    parameter.index >= 0 -> putValueArgument(parameter.index, argument)
    parameter == function.dispatchReceiverParameter -> dispatchReceiver = argument
    parameter == function.extensionReceiverParameter -> extensionReceiver = argument
    else -> error("Parameter ${parameter.dump()} not in function ${function.render()}")
}
